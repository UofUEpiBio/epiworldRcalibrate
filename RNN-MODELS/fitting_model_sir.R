...
incidence_part3 <- incidence[30:59] # Rows 41 to 59
...
incidence_part1 <- c(incidence_part1 ,rep(-1,59-length(incidence_part1 )))
incidence_part2 <- c(incidence_part2 ,rep(-1,59-length(incidence_part2 )))
incidence_part3 <- c(incidence_part3 ,rep(-1,59-length(incidence_part3 )))
...
time_series_input <- array(incidence_part1, dim = c(1, 59, 1))  # Reshape as required
...
ggplot() +
  ...
  # Add confidence interval ribbon for mean infected counts
  geom_ribbon(data = summary_stats_3, aes(x = date, ymin = lower_ci, ymax = upper_ci, fill = "Mean CI"), alpha = 0.4) +

  # Add mean infected line from `summary_stats`
  geom_line(data = summary_stats_3, aes(x = date, y = mean_count, color = "Mean Infected"), size = 0.6) +
